 ...
 * @since 1.0.16
 */
public class FakeComparableList<T> extends ListDelegate<T> implements Comparable<List<T>> {

	public FakeComparableList(List<T> delegate) {
		super( delegate );
	}

	public static <Source, Target> List<Target> convert(Collection<Source> o, Converter<Source, Target> converter) {
		if ( o == null ) {
			return null;
		} else if ( o instanceof List<?> list && converter == null ) {
			//noinspection rawtypes,unchecked
			return new FakeComparableList<>( (List) list );
		} else if ( converter == null ) {
			//noinspection rawtypes,unchecked
			return new FakeComparableList<>( (List) Arrays.asList( o.toArray() ) );
		} else {
			final ArrayList<Target> list = new ArrayList<>( o.size() );
			for ( Source s : o ) {
	...
	public int compareTo(List<T> o) {
		...
		return compare( (List<Comparable>) delegate, (List<Comparable>) o );
	}
