 ...
 * @since 1.0.0
 */
public class LoginProfileDataFetcher implements DataFetcher<AwsLoginProfile>, Serializable {
	...
	public DataFormat getDataFormat() {
		return DataFormats.componentMethodConvention( AwsLoginProfile.class, AwsConventionContext.INSTANCE );
	}

	@Override
	public List<AwsLoginProfile> fetch(DataFetchContext context) {
		try {
			List<AwsConnectorConfig.Account> accounts = AwsConnectorConfig.ACCOUNT.getAll( context );
			SdkHttpClient sdkHttpClient = AwsConnectorConfig.HTTP_CLIENT.find( context );
			List<? extends AwsUser> users = context.getSession().getOrFetch( AwsUser.class );
			List<AwsLoginProfile> list = new ArrayList<>();
